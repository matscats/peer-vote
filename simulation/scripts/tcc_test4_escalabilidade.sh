...
# Objetivo: Medir quantos votos conseguem ser incluídos no primeiro bloco após submissão
...
LOAD_LEVELS=(10 25 50 75 100 150 200 250 300)

echo "=========================================="
echo "TCC - Teste 4: Escalabilidade"
echo "=========================================="
echo ""
echo "Objetivo: Medir votos incluídos no primeiro bloco"
...
echo "=====================================" >> $RESULTS_FILE
echo "" >> $RESULTS_FILE
echo "Objetivo: Medir votos incluídos no primeiro bloco após submissão" >> $RESULTS_FILE
echo "Intervalo de blocos: 5s" >> $RESULTS_FILE
echo "" >> $RESULTS_FILE
...
for NUM_VOTERS in "${LOAD_LEVELS[@]}"; do
    ...
    echo "  Aguardando primeiro bloco..."
    ...
    while [ $ELAPSED -lt $TIMEOUT ]; do
        ...
        CURRENT_BLOCKS=$(grep "finalized successfully" logs/node1.log 2>/dev/null | wc -l | tr -d ' ')
        if [ $CURRENT_BLOCKS -gt $INITIAL_BLOCKS ]; then
            break
        fi
    done
    
    # Get votes in first block
    FIRST_BLOCK_LINE=$(grep "finalized successfully" logs/node1.log 2>/dev/null | tail -1)
    VOTES_IN_BLOCK=$(echo "$FIRST_BLOCK_LINE" | grep -o "with [0-9]* votes" | awk '{print $2}')
    if [ -z "$VOTES_IN_BLOCK" ]; then
        VOTES_IN_BLOCK=0
    fi
    ...
    if [ $VOTES_SUBMITTED -gt 0 ]; then
        INCLUSION_RATE=$(echo "scale=2; ($VOTES_IN_BLOCK / $VOTES_SUBMITTED) * 100" | bc)
    else
        INCLUSION_RATE="0.00"
    fi
    ...
    echo "    Votos no 1º bloco:    $VOTES_IN_BLOCK"
    echo "    Taxa de inclusão:     ${INCLUSION_RATE}%"
    echo "    Tempo até bloco:      ${ELAPSED}s"
    echo ""
    ...
    echo "  Votos no 1º bloco:    $VOTES_IN_BLOCK" >> $RESULTS_FILE
    echo "  Taxa de inclusão:     ${INCLUSION_RATE}%" >> $RESULTS_FILE
    echo "  Tempo até bloco:      ${ELAPSED}s" >> $RESULTS_FILE
    echo "" >> $RESULTS_FILE
